	...
	for (int i = 0; i < numParts; i++)
	{
		if (i == numParts - 1) // file
		{
			...
			if ((63 - freePosition) < 12)
			{
				...
				else
				{
					...
					createNewFileBlock(parentBlockNum);
					updateFreeBlock();
					blockRead(parentBlockNum, parentBlock);
					freePosition = endOfData(parentBlockNum);
				}
		...
		{ // dir
		  ...
		  // if dir does not exist create dir
			...
			unsigned char *noInfoBlock = malloc(60);
			memcpy(noInfoBlock, currentDirBlock + 4, 60);
			...
			for (int j = 0; j < numOfPossibleFiles; j++) // for each file/dir in dir
			{
				...
				if ((nextFile[0] == 0))
				{
					...
					createNewFileBlock(freeDirBlock);
...
void createNewFileBlock(int freeBlock)
{
...
int endOfData(int block)
{
	...
	for (int i = 4; numZeroes < 6 && i < 64; i++)
	{
		if (posBlock[i] == 0)
		{
			if (numZeroes == 0)
...
void list(char *result, char *directoryName)
{
	...
	unsigned char *noInfoBlock = malloc(60);
	memcpy(noInfoBlock, finalDirBlock + 4, 60);
	int numOfPossibleFiles = 5;
	if (parentBlockNum == 1)
	{ // root
		numOfPossibleFiles = 4;
...
int a2read(char *fileName, void *data, int length)
{
	...
	int numParts = 0;
	int parentBlockNum = 1;
	int startPos = 4; // should get from file
	...
	memcpy(data, currentFile + startPos, length - 1);
