	...
	for (int i = 0; i < numParts; i++)
	{
		...
		{ // dir
		  ...
		  // if dir does not exist create dir
			int overflowExists = 0;
			int p = 1;
			while (p == 1)
			{
				p = 0;
				overflowExists = checkOverflow(parentBlockNum);
				if (overflowExists > 0)
				{
					parentBlockNum = overflowExists;
					p = 1;
				}
				else
				{
			...
			unsigned char *noInfoBlock = malloc(54);
			memcpy(noInfoBlock, currentDirBlock + 10, 54);
