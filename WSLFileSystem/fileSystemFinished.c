...
int format(char *volumeName)
{
	...
	memset(rootDir, 0, 64);
	char *overflowBlock = "00020000100014";
...
int create(char *pathName)
{
	...
	for (int i = 0; i < numParts; i++)
	{
		if (i == numParts - 1) // file
		{
			...
			int HELLO = checkFileSize(parentBlockNum);
		...
		{ // dir
		  ...
		  // if dir does not exist create dir
			...
			{ // root
				...
				memcpy(noInfoBlock, currentDirBlock + 14, 56);
			}
...
void list(char *result, char *directoryName)
{
	...
	while (k == 1)
	{
		...
		if (parentBlockNum == 1)
		{ // root
			memcpy(noInfoBlock, finalDirBlock + 14, 56);
		}
		for (int j = 0; j < numOfPossibleFiles; j++) // for each file/dir in dir
		{
			int startChar = j * 12;
...
int a2write(char *fileName, void *data, int length)
{
	...
	if (blockRead(parentBlockNum, currentFile) < 0)
	{
		free(currentFile);
		return -1;
	}
	...
	updateFileSize(parentBlockNum, length);
	int beforeOverFlow = parentBlockNum;
	if (length > (roomLeft))
	{
		if (blockWrite(parentBlockNum, currentFile) < 0)
		{
			free(currentFile);
			return -1;
		}
		...
		if (blockRead(parentBlockNum, parentBlock) < 0)
		{
			free(currentFile);
			free(parentBlock);
			return -1;
		...
		unsigned char *currentFileAgain = realloc(currentFile, freePosition + length - roomLeft);
		if (currentFileAgain == NULL)
		{
			free(currentFile);
			free(parentBlock);
			return -1;
		}
		if (blockRead(parentBlockNum, currentFileAgain) < 0)
		{
			free(currentFile);
			free(parentBlock);
			free(currentFileAgain);
			return -1;
		}
		freePosition = endOfData(parentBlockNum);
		memcpy(currentFileAgain + freePosition, data + roomLeft, length - roomLeft);
		if (blockWrite(parentBlockNum, currentFileAgain) < 0)
		{
			free(currentFile);
			free(parentBlock);
			free(currentFileAgain);
	...
	updateFileSize(beforeOverFlow, length);
	return 0;
}
